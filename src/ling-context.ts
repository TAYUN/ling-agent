...
async function handleTurn(userMessage: string): Promise<string | undefined> {
  // /compact 命令：手动触发压缩
  if (userMessage.trim() === "/compact") {
    messages = await compactor.compact(messages);
    return "[ling] Conversation compacted.";
  }
  ...
  return "[ling] Reached max turns, stopping.";
}

// ===== 入口：支持 CLI 参数（非交互式）和 REPL（交互式）两种模式 =====

const inlineQuery = process.argv.slice(2).join(" ");

if (inlineQuery) {
  // 非交互式：从命令行参数读取问题，执行后退出
  handleTurn(inlineQuery)
    .then((result) => {
      console.log(`\n${result}\n`);
    })
    .catch((err) => {
  ...
  function prompt() {
    ...
    // 标准化提示符输出（强制 stdout 同步刷新）
    process.stdout.write(`[${historyTokens} tokens] > `);
  }
  ...
  rl.on("line", async (input) => {
    ...
    const output = await handleTurn(trimmed);
